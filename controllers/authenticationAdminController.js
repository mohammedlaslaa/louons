...
exports.postAuthAdmin = async (req, res) => {
  try {
    ...
    let admin = await Admin.findOne({ email: req.body.email, isActive: true });

    // If there is not admin with the email provided, send back a 400 response status code with a message.
...
// Validator of password (one letter uppercase and one special character minimum. One password must to have, one number minimum and its length must be at least 8 characters ) and valid email.
